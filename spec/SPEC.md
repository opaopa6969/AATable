...
- 空行はスキップ
- セパレータ行の判定: **ヘッダ行の直後の1行のみ**を候補とし、各セルが `-` / `:` / 空白のみで構成されかつ少なくとも1個の `-` を含む場合にセパレータとみなす（GFM 準拠）。ヘッダより前のセパレータ様式行は無視され、ヘッダ直後以降のデータ行は内容に関わらず保持される
- セルは `|` で分割後、先頭・末尾の空文字列要素を除去
- 各セルの前後の空白を除去

```python
# セパレータ行の判定例（ヘッダ直後の1行のみ）
"|---|---|"   → skip  (各セルが '-', ':' のみで '-' を含む)
"|:--|--:|"   → skip  (同上)
"| a | b |"   → data row
"|---|"       → data row (ヘッダ直後でなければセパレータにならない)
```
...
**前方参照対応**: `parse_mermaid()` は2パスで処理する。1パス目で全ノード定義（エッジ内のインライン定義含む）を `node_labels` に収集し、2パス目でエッジとスタンドアロン定義を出力する。これによりノード定義がエッジの後に書かれても（前方参照）ラベルがエッジに反映される。エッジで参照されたノード ID と同じスタンドアロン定義は重複出力を避けるため省略される。
